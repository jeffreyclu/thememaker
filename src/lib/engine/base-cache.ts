/**
 * The synchronous, same-origin BASE-COLOR CACHE — the flash-elimination layer.
 *
 * `chrome.storage` is async, so at `document_start` we can't know the theme before
 * the browser paints the site's original background. We avoid the reload flash by
 * caching the EXACT base background the engine painted onto html/body, in the
 * page's OWN `localStorage` (synchronous, same-origin). At the very top of
 * `document_start`, before any async read, the content script reads this and
 * early-paints that exact hex onto `<html>` — so the first frame is already themed.
 *
 ...
 * paints `roles.bg` directly, and this returns exactly that. Used as the FALLBACK
 * early paint when no per-origin cache exists yet (first themed load); once the
 * engine runs it caches the EXACT resolved base, so later loads read that instead
 * and the early paint matches the final paint.
 *
