...
### [medium] Module-level mutable `memoryCache` singleton (shared global state)
...
### [low] `paletteFromApiResponse` discards the API's harmony and only uses `hexes[0]` as a local seed
...
### [low] In-memory and persistent caches can diverge on the fallback path (correctly) but the "don't cache fallback" rule is implicit
...
### [nit] `apiSchemeUrl` default `count = 6` is unused-ish given only `hexes[0]` matters
