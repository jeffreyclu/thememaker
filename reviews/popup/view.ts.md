...
- CONFIRMED FIXED: `schemeSwatches` (view.ts:232-243) iterates `Object.values(scheme.colors)` cast-free, dedups via `seen`.
- VERIFIED-INVALID re-check (use palette.swatches instead): AGREE the verdict is correct. Verified the equivalence claim at the source: `palette.swatches = themeColors.map(tc => tc.color)` (palette.ts:392) and `schemeFromPalette` builds `colors[tc.role] = tc.color` from the SAME `themeColors` (engine-bridge.ts:74-76). So `Object.values(scheme.colors)` is exactly the swatch color set. Switching sources would add a legacy branch for zero display change. Genuinely invalid.
- Re-render gating (NEW scrutiny — no regression found): the WeakMap-keyed gate (view.ts:387-406) rebuilds details only on `current`/`overrides` change, favorites on `favorites`, history on `history`. Traced every reducer action: the only producers of NEW `current`/`overrides`/`history`/`favorites` references are the actions that should rebuild (generate/applyFavorite/selectHistory/setOverride/clearOverride(s)/reset/setFavorites/hydrate). `selectIntensity`/toggles/`applied`/`setSiteEnabled`/`setPicking` keep those references, so lists + focus survive a slider drag — the intended fix, no stale-DOM or stale-handler case found. Cheap attribute/visibility updates (disabled/hidden/aria) run unconditionally every render, so disclosure state never goes stale.
- NEW (NOTE — colors robustness): `schemeSwatches` also unguarded against a `.colors`-less legacy scheme in history/favorites; crashes inside `renderHistory`/`renderFavorites`. Same known/accepted root cause.
