...
- CONFIRMED FIXED (persist-queue serialization): `applyAndPersist` (240-245) chains each persist onto `persistQueue` with a `.catch(() => {})` so a failed persist can't break the chain; `persistSession` (203-231) read-modify-writes AFTER the prior write commits. No lost-update / interleave found. Subtle (benign): `persistSession(s)` reads the LIVE session object at execution time, so a queued persist writes the latest state rather than the enqueue-time snapshot — this CONVERGES to the final state and cannot lose an update. OK.
- CONFIRMED FIXED (session init without placeholder cast): `showPicker` (267-321) builds `panel`/`pick` whose handlers close over the `const session` declared below; handlers only fire on later user interaction (past TDZ), so the assemble-once pattern is sound. No `as` cast on the session. Verified.
- `colors` build site (218-229): always writes a `colors` field — `{}` when there's no prior saved scheme, else preserves the existing one. So the content script never PRODUCES a `.colors`-less scheme (no crash introduced here). NOTE: the picker is only reachable when the popup already has a `current` (popup gate in `onPickElement`), which means a real popup-written `savedScheme` (real `colors`) already exists by the time `persistSession` runs — so `colors: {}` is effectively dead/defensive. Preserved `colors` stays consistent because the picker never changes the palette (only `<tag>|<prop>` overrides). No display drift.
- Teardown: `hidePicker` removes the keydown listener, stops pick, destroys the panel, nulls `picker`. A persist queued before teardown still completes against the captured session (writes the final picks + `enabled:true`) — desired, not a leak. No teardown race found.
